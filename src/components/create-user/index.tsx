...
export default function CreateUser({ open, setOpen }: { open: boolean; setOpen: React.Dispatch<React.SetStateAction<boolean>> }) {
  ...
  const validateAllInputs = useCallback(() => {
    ...
    setValidInputs(
      validateEmail(userData.email) &&
        ...
        userData.birthDate <=
          new Date(currentDate.current.getFullYear() - requiredAge, currentDate.current.getMonth(), currentDate.current.getDate()) && // Check if birthDate is before the required age
  ...
  function userHasMinimumAge() {
    return (
      requiredAge &&
      requiredAge > 0 && // Check if requiredAge is valid
